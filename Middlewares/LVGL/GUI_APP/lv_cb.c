...
void sys_btn_event_cb(lv_event_t * e)
{
    ...
    //Step_Motor
    else if(target == Step_Motor_btn_speed_up)                      /* 加速按钮 */
    {
        Step_Motor_speed_val += 30;
        LED1 = !LED1;
    }
    else if(target == Step_Motor_btn_speed_down)               /* 减速按钮 */
    {
        Step_Motor_speed_val -= 30;
        LED1 = !LED1;
    }
    ...
    else if(target == Servo_btn_back)                     /* 返回按钮 */
    {
        ...
        lv_label_set_text_fmt(DC_Motor_label_speed, "DC_Motor_Speed : %d RPM", DC_Motor_speed_val);    /* 更新速度值 */
        lv_label_set_text_fmt(Step_Motor_label_speed, "Step_Motor_Speed : %d RPM", Step_Motor_speed_val);    /* 更新速度值 */
        lv_label_set_text_fmt(Bldc_Motor_label_speed, "Bldc_Motor_Speed : %d RPM", Bldc_Motor_speed_val);    /* 更新速度值 */
        lv_label_set_text_fmt(Servo_label_speed, "Servo_Speed : %d RPM", Servo_speed_val);    /* 更新速度值 */
}
